...
class NonLocalBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        phi = phi.view(batch_size, self.in_channels // 2, -1).permute(0, 2, 1)
        g = g.view(batch_size, self.in_channels // 2, -1).permute(0, 2, 1)
        
        sim = torch.matmul(theta, phi.permute(0, 2, 1))
        sim = sim / (self.in_channels // 2) ** 0.5
...
class HybridGNet(nn.Module):
    def __init__(self):
        ...
        # Zmniejszamy dropout z 0.2 do 0.1
...
for epoch in range(EPOCHS):
    ...
    if avg_val_dice > best_dice:
        ...
        torch.save(model.state_dict(), "best_hybridgnet_model.pth")

# --- TEST ---
model.load_state_dict(torch.load("best_hybridgnet_model.pth"))
model.eval()
...
for i, metric in enumerate(metrics):
    ...
    plt.legend()
plt.tight_layout()
plt.savefig('hybridgnet_metrics.png')
